...
def append_settings_py():
    ...
    return REPO_NAME, PORJ_NAME, PUBLIC_IP


def config_gunicorn(REPO_NAME, PROJ_NAME, PUBLIC_IP):
    gunicorn_output = '''
    ...
    '''
    print(gunicorn_output)
    wait = raw_input("\nLooking for the above output, press ENTER to continue")
    os.system("gunicorn --bind 0.0.0.0:8000 {}.wsgi:application".format(PORJ_NAME))

    os.system("deactivate")

    placeholders = {"repoName": REPO_NAME, "projectName": PORJ_NAME, "public_ip": PUBLIC_IP}
    gunicorn_service = '''
    [Unit]
...
def config_nginx():
    ...
    nginx_file += "\t}\n}"
    os.system("sudo echo '{}' >> /etc/nginx/sites-available/{}".format(nginx_file, PORJ_NAME))

    os.system("sudo ln -s /etc/nginx/sites-available/{} /etc/nginx/sites-enabled".format(PORJ_NAME))
...
def perpare_server_for_app():
    ...
    '''
    REPO_NAME, PROJ_NAME, PUBLIC_IP = append_settings_py()

    config_gunicorn(REPO_NAME, PROJ_NAME, PUBLIC_IP)
    config_nginx(REPO_NAME, PROJ_NAME, PUBLIC_IP)
